...
"Safe default: /bin/zsh ./ip-quality.zsh [--scope SCOPE]"
"安全默认：不加确认参数时只显示查询计划，不访问网络"
"Live query: /bin/zsh ./ip-quality.zsh --confirm-network-lookup [--scope SCOPE] [-4|-6]"
"            --scope reputation|dnsbl|media-ai|mail|full"
...
smail[unknown]="$Font_Suffix$Font_Purple未知 $Font_B"
...
print -ru2 -- "Use the supervised network-manager egress workflow for proxy paths."
return 64
;;
--)shift
legacy_args+=("$@")
...
case "$query_scope" in
reputation|dnsbl|media-ai|mail|full) ;;
...
scope_includes media-ai&&required+=(dig nslookup gunzip)
...
is_score_0_to_100 "0"&&is_score_0_to_100 "100"&&! is_score_0_to_100 "null"&&! is_score_0_to_100 "101"||{
...
maxmind=()
local RESPONSE=$(curl_safe $CurlARG -Ls -$1 -m 10 "https://ipinfo.check.place/$IP?lang=$YY")
...
if [[ $YY != "en" ]];then
local backup_response=$(curl_safe $CurlARG -s -$1 -m 10 "https://ipinfo.check.place/$IP?lang=en")
...
scamalytics=()
local RESPONSE=$(curl_safe $CurlARG -sL -$1 -m 10 "https://ipinfo.check.place/$IP?db=scamalytics")
echo "$RESPONSE"|jq . >/dev/null 2>&1||RESPONSE=""
...
if ! is_score_0_to_100 "${scamalytics[score]}";then
scamalytics[score]=""
scamalytics[risk]=""
elif [[ ${scamalytics[score]} -lt 20 ]];then
...
ipapi=()
if [[ $IP == *:* ]];then
local RESPONSE=$(curl_safe -Ls -m 10 -H 'origin: https://ipapi.is' "https://api.ipapi.is/?q=$IP")
else
local RESPONSE=$(curl_safe $CurlARG -sL -m 10 -H 'origin: https://ipapi.is' "https://api.ipapi.is/?q=$IP")
fi
echo "$RESPONSE"|jq . >/dev/null 2>&1||RESPONSE=""
...
abuseipdb=()
local RESPONSE=$(curl_safe $CurlARG -sL -$1 -m 10 "https://ipinfo.check.place/$IP?db=abuseipdb")
echo "$RESPONSE"|jq . >/dev/null 2>&1||RESPONSE=""
...
if ! is_score_0_to_100 "${abuseipdb[score]}";then
abuseipdb[score]=""
abuseipdb[risk]=""
...
show_progress_bar "$temp_info" $((40-12-${sinfo[ldatabase]}))
ip2location=()
local RESPONSE=$(curl_safe $CurlARG -sL -$1 -m 10 "https://ipinfo.check.place/$IP?db=ip2location")
echo "$RESPONSE"|jq . >/dev/null 2>&1||RESPONSE=""
...
if ! is_score_0_to_100 "${ip2location[score]}";then
ip2location[score]=""
ip2location[risk]=""
elif [[ ${ip2location[score]} -lt 33 ]];then
...
show_progress_bar "$temp_info" $((40-7-${sinfo[ldatabase]}))
ipdata=()
local RESPONSE=$(curl_safe $CurlARG -sL -$1 -m 10 "https://ipinfo.check.place/$IP?db=ipdata")
echo "$RESPONSE"|jq . >/dev/null 2>&1||RESPONSE=""
...
ipqs=()
local RESPONSE=$(curl_safe $CurlARG -sL -$1 -m 10 "https://ipinfo.check.place/$IP?db=ipqualityscore")
echo "$RESPONSE"|jq . >/dev/null 2>&1||RESPONSE=""
ipqs[score]=$(echo "$RESPONSE"|jq -r '.fraud_score')
if ! is_score_0_to_100 "${ipqs[score]}";then
ipqs[score]=""
ipqs[risk]=""
elif [[ ${ipqs[score]} -lt 75 ]];then
...
local Ftmpresult=$(curl_safe $CurlARG -$1 --user-agent "$UA_Browser" -sL -m 10 "https://www.tiktok.com/")
[[ $Ftmpresult == *"Please wait..."* ]]&&Ftmpresult=$(curl_safe $CurlARG --user-agent "$UA_Browser" -sL -m 10 "https://www.tiktok.com/explore")
...
local STmpresult=$(curl_safe $CurlARG -$1 --user-agent "$UA_Browser" -sL -m 10 -H "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9" -H "Accept-Encoding: gzip" -H "Accept-Language: en" "https://www.tiktok.com"|gunzip 2>/dev/null)
[[ $Ftmpresult == *"Please wait..."* ]]&&STmpresult=$(curl_safe $CurlARG --user-agent "$UA_Browser" -sL -m 10 -H "Accept: text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9" -H "Accept-Encoding: gzip" -H "Accept-Language: en" "https://www.tiktok.com/explore"|gunzip 2>/dev/null)
...
local result1=$(curl_safe $CurlARG -$1 --user-agent "$UA_Browser" -fsL -X GET --max-time 10 --tlsv1.3 "https://www.netflix.com/title/81280792" 2>&1)
local result2=$(curl_safe $CurlARG -$1 --user-agent "$UA_Browser" -fsL -X GET --max-time 10 --tlsv1.3 "https://www.netflix.com/title/70143836" 2>&1)
...
local result3=$(Check_DNS_3 $checkunlockurl)
local resultunlocktype=$(Get_Unlock_Type $result1 $result3)
local tmpresult=$(curl_safe $CurlARG -$1 --max-time 10 -sSL -H "Accept-Language: en" "https://www.youtube.com/premium" 2>&1)
...
local result3=$(Check_DNS_3 $checkunlockurl)
local resultunlocktype=$(Get_Unlock_Type $result1 $result3)
local tmpresult=$(curl_safe $CurlARG -$1 --user-agent "$UA_Browser" -sL --max-time 10 "https://www.primevideo.com" 2>&1)
if [[ $tmpresult == "curl"* ]];then
...
resp=$(curl_safe $CurlARG -$1 -fsL --user-agent "$UA_Browser" --max-time 10 $resolve_opt --write-out '\n%{http_code}' "https://www.reddit.com/")
...
local countryCode="$(curl_safe $CurlARG --max-time 10 -sS https://chat.openai.com/cdn-cgi/trace 2>&1|grep "loc="|awk -F= '{print $2}')"
...
printf 'QUIT\r\n'|nc -s "$IP" -w "$wait_seconds" "$host" "$port" 2>&1
}
check_email_service(){
local service=$1
local port=25
...
calc_padding "${shead[command]}" 72
echo -ne "\r$PADDING${shead[command]}\n"
...
echo -ne "\r$Font_Cyan${smedia[status]}${tiktok[ustatus]}${netflix[ustatus]}${youtube[ustatus]}${amazon[ustatus]}${reddit[ustatus]}${chatgpt[ustatus]}$Font_Suffix\n"
echo -ne "\r$Font_Cyan${smedia[region]}$Font_Green${tiktok[uregion]}${netflix[uregion]}${youtube[uregion]}${amazon[uregion]}${reddit[uregion]}${chatgpt[uregion]}$Font_Suffix\n"
echo -ne "\r$Font_Cyan${smedia[type]}${tiktok[utype]}${netflix[utype]}${youtube[utype]}${amazon[utype]}${reddit[utype]}${chatgpt[utype]}$Font_Suffix\n"
}
show_mail(){
...
set_language
[[ "$query_scope" == "dnsbl" && $IPV4check -eq 0 ]]&&ERRORcode=61
